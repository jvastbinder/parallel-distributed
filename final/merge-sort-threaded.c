...
void merge(int *arr, int low, int mid, int high)
{
  ...
  /* 
   * merge sorted halves and store in original array
   */
  int i = 0;
  int j = 0;
  int k = low;
  ...
  /* 
   * if either side is longer, append it to the end of the section
   */
  while (i < left_length) {
	arr[k++] = left[i++];
  }
...
merge_sort(int tid, int *arr, int num_elts, int low, int high)
{
  ...
  /* 
   * recursively separate lists until each half is only one element
   *
   * then merge the returned list halves until all the recursive calls are resolved
   */
  if (low < high) {
...
/* p_threads takes a struct of stuff as an argument */
typedef struct {
  int tid;
  int num_threads;
  int num_elts;
...
/* p_threads takes a struct of stuff to return things to */
typedef struct {
  int tid;
  int elts_per_thread;
  int low;
...
merge_sort_thread(void *in)
{
  ...
  /* 
   * for n elements and t threads, each thread sorts n/t elements
   */
  ...
  /* ---------------- */
  merge_sort(args->tid, arr, args->num_elts, low, high);
